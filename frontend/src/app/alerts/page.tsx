...
import { Alert, Incident, IncidentSeverity } from "../../types";
import {
  Bell,
  Search,
  Filter,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Real-time telemetry ingestion, deduplication fingerprinting, and 30-minute automated incident clustering.
          </p>
        </div>
          ...
          <div className="flex items-center gap-1 bg-slate-950 p-1 rounded-lg border border-slate-800">
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${
                tab === "ALL" ? "bg-blue-600 text-white shadow" : "text-slate-400 hover:text-slate-200"
              }`}
            >
              All Alerts ({total})
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${
                tab === "UNASSIGNED" ? "bg-blue-600 text-white shadow" : "text-slate-400 hover:text-slate-200"
              }`}
            >
              Unassigned Stream
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${
                tab === "CORRELATED" ? "bg-blue-600 text-white shadow" : "text-slate-400 hover:text-slate-200"
              }`}
            >
              Correlated Clusters
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-950 border border-slate-700 rounded-lg text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500"
            />
          </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl">
        {loading ? (
          ...
          <div className="p-12 text-center text-rose-300 space-y-2">
          ...
          <div className="p-16 text-center text-slate-400 space-y-2">
            ...
            <p className="text-base font-semibold text-slate-300">No alerts found</p>
                ...
                >
                  <div className="space-y-1.5 flex-1">
                    ...
                    {hasIncident ? (
                      <div className="inline-flex items-center gap-2 pt-1 text-xs">
                        ...
                        <Link
                          href={`/incidents/${hasIncident}`}
                          className="font-semibold text-blue-400 hover:text-blue-300 flex items-center gap-1 bg-blue-950/60 border border-blue-800/80 px-2 py-0.5 rounded"
                        >
                          <Layers className="h-3 w-3" />
                          {incidentObj?.title || `Incident #${hasIncident}`}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="px-2.5 py-1.5 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium border border-slate-700 flex items-center gap-1"
                    ...
                    {!hasIncident && (
                      <button
                        onClick={() => {
                          ...
                          setTargetIncidentId(activeIncidents[0]?._id || "");
                        }}
        ...
        {totalPages > 1 && (
          <div className="p-4 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
            <div>
              ...
              <span className="text-white font-semibold">{totalPages}</span> ({total} alerts)
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setPage((p) => Math.max(1, p - 1))}
                disabled={page <= 1}
                className="px-3 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-200 disabled:opacity-40"
              >
                Previous
              </button>
              <button
            ...
            {simResult && (
              ...
              >
                <div className="font-bold flex items-center gap-1.5">
                  ...
                  {simResult.escalated && (
                    <span className="ml-2 px-2 py-0.5 rounded bg-rose-900 text-rose-200 text-[10px]">
                      SEVERITY ESCALATED
                    </span>
                  )}
                </div>
                {simResult.incident && (
                  <p>
                    Target Incident:{" "}
                    <Link
                      href={`/incidents/${simResult.incident._id}`}
                      className="underline font-mono font-semibold"
                    >
                      {simResult.incident.title} (#{simResult.incident._id})
                    </Link>
                  </p>
                )}
                <p className="text-[11px] opacity-80">
                  {simResult.action === "DEDUPLICATED"
                    ...
                    : simResult.action === "CORRELATED_TO_EXISTING"
                    ? "Matched active non-resolved incident in 30-min window for this service."
                    : "No active incident for this service within 30 minutes. Auto-created new incident."}
                </p>
              </div>
            )}
            ...
            <form onSubmit={handleSimulateAlert} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full mt-1 px-3 py-2 bg-slate-950 border border-slate-700 rounded text-xs text-slate-100 focus:outline-none focus:border-blue-500"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-slate-300">Source Monitor</label>
                  <input
                    type="text"
                    value={simSource}
              ...
              <div className="pt-2 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setIsSimulateOpen(false)}
                  className="px-3.5 py-1.5 rounded bg-slate-800 text-slate-300 text-xs font-semibold"
                >
                  Close
                </button>
                <button
      ...
      {associateAlert && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-md p-6 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="text-base font-bold text-slate-100">Associate Alert to Incident</h3>
              <button
            ...
            <div className="p-3 bg-slate-950 rounded border border-slate-800 text-xs space-y-1">
              <p className="font-semibold text-slate-200">{associateAlert.title}</p>
              <p className="text-slate-400">
                Service: <span className="font-mono text-slate-300">{associateAlert.service}</span> | Severity:{" "}
                <span className="font-mono text-slate-300">{associateAlert.severity}</span>
              </p>
            </div>

            <div>
              <label className="text-xs font-semibold text-slate-300">Choose Active Incident</label>
              <select
                value={targetIncidentId}
                onChange={(e) => setTargetIncidentId(e.target.value)}
                className="w-full mt-1.5 px-3 py-2 bg-slate-950 border border-slate-700 rounded text-xs text-slate-100 focus:outline-none focus:border-blue-500"
              >
      ...
      {detailAlert && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-2xl p-6 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl space-y-4 max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="text-base font-bold text-slate-100 flex items-center gap-2">
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <p className="text-sm font-bold text-slate-100">{detailAlert.title}</p>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-3 bg-slate-950 rounded-lg border border-slate-800">
                <div>
                  ...
                  <p className="font-mono text-slate-200">{detailAlert.service}</p>
                </div>
                <div>
                  <span className="text-slate-500 text-[10px] uppercase font-semibold">Severity</span>
                  <p className="font-mono text-slate-200">{detailAlert.severity}</p>
                </div>
                <div>
                  <span className="text-slate-500 text-[10px] uppercase font-semibold">Source</span>
                  <p className="font-mono text-slate-200">{detailAlert.source}</p>
                </div>
                <div>
                  <span className="text-slate-500 text-[10px] uppercase font-semibold">Occurrences</span>
                  <p className="font-mono text-slate-200">{detailAlert.count || 1}x</p>
                </div>
              </div>
              ...
              {detailAlert.fingerprint && (
                <div>
                  ...
                  <p className="p-2 bg-slate-950 rounded border border-slate-800 font-mono text-[11px] text-purple-300 break-all">
                    {detailAlert.fingerprint}
                  </p>
                </div>
              )}
              ...
              <div>
                ...
                <pre className="p-3 bg-slate-950 rounded border border-slate-800 font-mono text-[11px] text-slate-300 overflow-x-auto max-h-48">
            ...
            <div className="flex justify-end pt-2">
              <button
                onClick={() => setDetailAlert(null)}
                className="px-4 py-1.5 rounded bg-slate-800 text-slate-200 text-xs font-semibold"
              >
                Close
              </button>
            </div>
